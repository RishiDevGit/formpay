...
const MobileNavList=(props)=>{
    const {open,setOpen,options,currentActiveItem,setCurrentActiveItem}={...props};

const accordionIds = [{title:"Resources",id:"resources",content:
<ContentPlaceholder />},{title:"Company",id:"company",content:
            <ContentPlaceholder />}];
    return (
        <div
        className={
      ...
      >
        <div className=" bg-white divide-y-2 divide-gray-50">
          ...
          <button class="mr-6 bg-formPay100 rounded-2 hover:bg-gray-100 text-smallText text-white font-medium py-2 px-4">
            ...
            {options.map(item=> !(["resources","company"].includes(item?.id))? <a
                  href="#"
                  className="-m-3 p-3 flex items-center rounded-md hover:bg-gray-50"
                >
              
                  <span className="ml-3 text-normalGrey font-fontLight">
                    {item?.title}
                  </span>
                </a>:<></> )}    
